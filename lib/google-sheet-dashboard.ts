...
const SHEETS={projects:888299704,wa:172604510,rcs:269889098} as const;
...
export async function loadGoogleSheetDashboard(){
  const [projects,wa,rcs]=await Promise.all([fetchCsv(SHEETS.projects),fetchCsv(SHEETS.wa),fetchCsv(SHEETS.rcs)]);
  return buildDashboard(projects,wa,rcs,SOURCE_SPREADSHEET_URL);
}
