...
export function buildDashboard(projectCsv:string,rmCsv:string,waCsv:string,rcsCsv:string,sourceUrl:string){
 ...
 const actualRows=[...actual.values()],waMonths=[...new Set(actualRows.filter(x=>x.waRevenue||x.waCost).map(x=>x.month))].sort().reverse().slice(0,3),rcsMonths=[...new Set(actualRows.filter(x=>x.rcsRevenue||x.rcsCost).map(x=>x.month))].sort().reverse().slice(0,3),waDivisor=Math.max(1,waMonths.length),rcsDivisor=Math.max(1,rcsMonths.length);new Set(actualRows.map(x=>x.client)).forEach(client=>{const waHistory=actualRows.filter(x=>x.client===client&&waMonths.includes(x.month)),rcsHistory=actualRows.filter(x=>x.client===client&&rcsMonths.includes(x.month));if(!waHistory.length&&!rcsHistory.length)return;const waRate=waHistory.reduce((a,x)=>({revenue:a.revenue+x.waRevenue,cost:a.cost+x.waCost}),{revenue:0,cost:0}),rcsRate=rcsHistory.reduce((a,x)=>({revenue:a.revenue+x.rcsRevenue,cost:a.cost+x.rcsCost}),{revenue:0,cost:0});future.forEach(month=>{const key=client+'\u0000'+month,x=forecast.get(key)||empty(client,month,'Forecast');x.waRevenue+=waRate.revenue/waDivisor;x.waCost+=waRate.cost/waDivisor;x.rcsRevenue+=rcsRate.revenue/rcsDivisor;x.rcsCost+=rcsRate.cost/rcsDivisor;forecast.set(key,x)})});
