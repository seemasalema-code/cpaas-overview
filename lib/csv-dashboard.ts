...
const empty=(client:string,month:string,periodType:'Actual'|'Forecast'='Actual'):Monthly=>({client,month,chatbotRevenue:0,chatbotCost:0,waRevenue:0,waCost:0,rcsRevenue:0,rcsCost:0,periodType});
...
export function buildDashboard(projectCsv:string,rmCsv:string,waCsv:string,rcsCsv:string,sourceUrl:string){
 ...
 if(ri>=0){const h=rm[ri],cc=col(h,['Client']),cb=col(h,['Chatbot','Project','Project Name']),cm=col(h,['Month','Billing Month','Revenue Month']),cr=col(h,['Total Netcore Revenue','Total Revenue Netcore','Total Revenue','Monthly Revenue','Revenue']),co=col(h,['Total Vendor Cost','Total Cost Vendor','Total Cost','Vendor Cost','Cost']);if([cc,cb,cm,cr,co].some(i=>i<0))throw new Error('Chatbot R&M is missing a required named financial column.');
  rm.slice(ri+1).forEach(r=>{const client=clean(r[cc]),bot=clean(r[cb]),month=monthValue(r[cm]);if(!client||!bot||!month||month<START)return;const revenue=num(r[cr]),cost=num(r[co]);if(!revenue&&!cost)return;const bk=rk(bot);
 ...
 projects.forEach(p=>{const f=projectTotals.get(rk(p.project));if(f){p.revenue=f.revenue;p.cost=f.cost;p.margin=f.revenue-f.cost}});
 ...
 const addUsage=(rows:string[][],kind:'wa'|'rcs')=>{const i=rows.findIndex(r=>r.some(v=>['client','client name','client legal name'].includes(hk(v)))&&r.some(v=>hk(v)==='month'));if(i<0)return;const h=rows[i],cc=col(h,['Client','Client Name','Client Legal Name']),cm=col(h,['Month','Billing Month','Revenue Month']),cr=col(h,['Revenue','Total Revenue','Monthly Revenue']),co=col(h,['Cost','Total Cost','Monthly Cost']);if([cc,cm,cr,co].some(x=>x<0))return;rows.slice(i+1).forEach(r=>{const client=clean(r[cc]),month=monthValue(r[cm]);if(!client||!month||month<START||month>current)return;const key=client+'\u0000'+month,x=actual.get(key)||empty(client,month);if(kind==='wa'){x.waRevenue+=num(r[cr]);x.waCost+=num(r[co])}else{x.rcsRevenue+=num(r[cr]);x.rcsCost+=num(r[co])}actual.set(key,x)})};
 addUsage(wa,'wa');addUsage(rcs,'rcs');
 const actualRows=[...actual.values()];new Set(actualRows.map(x=>x.client)).forEach(client=>{const history=actualRows.filter(x=>x.client===client).sort((a,b)=>b.month.localeCompare(a.month)).slice(0,3);if(!history.length)return;const rate=history.reduce((a,x)=>({waRevenue:a.waRevenue+x.waRevenue,waCost:a.waCost+x.waCost,rcsRevenue:a.rcsRevenue+x.rcsRevenue,rcsCost:a.rcsCost+x.rcsCost}),{waRevenue:0,waCost:0,rcsRevenue:0,rcsCost:0}),d=history.length;future.forEach(month=>{const key=client+'\u0000'+month,x=forecast.get(key)||empty(client,month,'Forecast');x.waRevenue+=rate.waRevenue/d;x.waCost+=rate.waCost/d;x.rcsRevenue+=rate.rcsRevenue/d;x.rcsCost+=rate.rcsCost/d;forecast.set(key,x)})});
 ...
 const forecastMonthly=future.map(month=>forecastClientMonthly.filter(x=>x.month===month).reduce((a,x)=>({...a,chatbotRevenue:a.chatbotRevenue+x.chatbotRevenue,chatbotCost:a.chatbotCost+x.chatbotCost,waRevenue:a.waRevenue+x.waRevenue,waCost:a.waCost+x.waCost,rcsRevenue:a.rcsRevenue+x.rcsRevenue,rcsCost:a.rcsCost+x.rcsCost,totalRevenue:a.totalRevenue+x.chatbotRevenue+x.waRevenue+x.rcsRevenue,totalCost:a.totalCost+x.chatbotCost+x.waCost+x.rcsCost}),{month,periodType:'Forecast' as const,chatbotRevenue:0,chatbotCost:0,waRevenue:0,waCost:0,rcsRevenue:0,rcsCost:0,totalRevenue:0,totalCost:0}));
